...
for split in ['even', 'odd']:
    ...
    skim_file = opt.skimFile
    suffix = skim_file.split('/')[1]
    ...
    # Input variables
    input_vars = [
        'DRll', 'DRtwoAK4bjets', 'InvMak4bJJ', 'InvMll', 'MET', 'ak4jet0eta',
        'ak4jet0pt', 'ak4jet1eta', 'ak4jet1pt', 'ak8bjet_eta', 'ak8bjet_pt',
        'ak8jeteta', 'ak8jetpt', 'lepton0Cone_pt', 'lepton1Conept', 'nAK4',
        'nAK4bJet', 'nAK8bJet', 'nak8',
    ]

    print(f'Using {len(input_vars)} input variables')

    print(f'Using skim file {skim_file}')

    # Load dataframe from parquet file
    df = pd.read_parquet(skim_file)
    ...
    assert len(set(tags).intersection(set(pd.unique(df['tag'])))) == len(
        tags)  # Just cross check to avoid mistakes
    ...
    print(
        f"Number of events with negative weight is : {Nevents_with_negative_weight}")
    ...
    if (df['event_weight'] < 0).sum() > 0:
        raise RuntimeError(
            f"There are {(df['event_weight'] < 0).sum()} events with negative event weight, this should not happen")
    ...
    for tag in df.tag.unique():
        ...
        df.loc[df['tag'] == tag, 'training_weight'] *= df.shape[0] / \
            df[df['tag'] == tag]['event_weight'].sum()
    df
    ...
    def checkBatches(df, label_column, batch_size=128, weight_column='weight'):
        ...
        for i in range(N_checks):
            ...
            for label in labels:
                sum_label[label] += rnd_df[rnd_df[label_column]
                                           == label][weight_column].sum()
                N_label[label] += rnd_df[rnd_df[label_column]
                                         == label][weight_column].shape[0]
        print(f'On average, per batch the total weight is')
        for label in labels:
            sum_label[label] /= N_checks
            N_label[label] /= N_checks
            print(
                f'\t... {label:20s}: {sum_label[label]:15.9f} [{N_label[label]} events]')

    print('Using event weight')
    checkBatches(df, label_column='tag', weight_column='event_weight',
                 batch_size=parameters['batch_size'])
    print('Using training weight')
    checkBatches(df, label_column='tag', weight_column='training_weight',
                 batch_size=parameters['batch_size'])

    # Plot the background and signal weights #
    matplotlib.use(
        'Agg')  # to avoid """qt.qpa.plugin: Could not find the Qt platform plugin "xcb" in "" """ error
    ...
    for irow, tag in enumerate(tags):
        for icol, column in enumerate(['event_weight', 'training_weight']):
            axs[irow, icol].hist(df[df['tag'] == tag]
                                 [column], bins=100, color='b')
            axs[irow, icol].set_title(f"Category = {tag}")
            axs[irow, icol].set_xlabel(column)
            axs[irow, icol].set_yscale('log')
    fig.savefig(f"{output_dir}/event_weights_A_{split}.pdf", dpi=300)

    # Determine splitting variable #
    split_var = np.abs(df['MET'].copy())
    split_var *= 1e4
    split_var -= np.floor(split_var)
    split_var = (split_var*1e1).astype(int)
    split_var = split_var % 2 == 0
    print(
        f'Even set has {df[split_var].shape[0]:10d} events [{df[split_var].shape[0]/df.shape[0]*100:5.2f}%]')
    print(
        f'Odd  set has {df[~split_var].shape[0]:10d} events [{df[~split_var].shape[0]/df.shape[0]*100:5.2f}%]')
    ...
    print('\nFinal sets')
    print(
        f'Training set   = {train_df.shape[0]} ({train_df.shape[0] / df.shape[0] * 100 :2.2f}%)')
    print(
        f'Validation set = {val_df.shape[0]} ({val_df.shape[0] / df.shape[0] * 100 :2.2f}%)')
    print(
        f'Testing set    = {test_df.shape[0]} ({test_df.shape[0] / df.shape[0] * 100 :2.2f}%)')
    ...
    normalizer = preprocessing.Normalization(mean=train_df[input_vars].mean(axis=0).to_numpy(),
                                             variance=train_df[input_vars].var(
                                                 axis=0).to_numpy(),
    ...
    History.PlotHistory(loss_history, params=parameters,
                        outputName=f'{output_dir}/loss_{suffix}_{split}.png')
    ...
    # Numerator #
    num = pd.DataFrame((test_df[[output_tags[i] for i in range(
        len(tags)) if i in signal_idx]]).sum(axis=1))
    # Denominator #
    den = pd.DataFrame(test_df[[output_tags[i] for i in range(
        len(tags)) if i not in signal_idx]].sum(axis=1))
    # Ln #
    d_HH = np.log(num / den)
    test_df['d_HH'] = d_HH
    print("ROC curves")
    ...
    for tag in tags:
        print(
            f'ROC curve of binary classification of {tag} node versus all the others')
        roc.rocAndSig(y_true=test_df[tag],
                      ...
                      outputName=f'{output_dir}/roc_{suffix}_{split}_{tag}.pdf')
    ...
    roc.rocAndSig(y_true=test_df['HH'],
                  ...
                  outputName=f'{output_dir}/roc_{suffix}_{split}_d_HH.pdf')
    ...
    for tag in tags:
        ...
        roc.multiRoc(outputs=[test_df[test_df['tag'] == tag][f'output {tag}'] for tag in tags_order],
                     ...
                     outputName=f'{output_dir}/multi_roc_{suffix}_{split}_output_{tag}.pdf')
    ...
    for irow, output_tag in enumerate(output_tags+['d_HH']):
        for icol, weight in enumerate(['event_weight', 'training_weight']):
            ...
            tag_content = {tag: get_bin_content(
                bins, tag_df[tag][output_tag], tag_df[tag][weight])for tag in tags}
            tag_cumsum_left = {tag: np.cumsum(
                tag_content[tag])/tag_content[tag].sum() for tag in tags}
            tag_cumsum_right = {tag: np.cumsum(
                tag_content[tag][::-1])[::-1]/tag_content[tag].sum() for tag in tags}
            ...
            for i, (tag, content) in enumerate(tag_content.items()):
                axs[irow, icol].bar(
                    x=centers, height=content, width=widths, fill=False, edgecolor=colors[i], label=tag)
            ...
            axs[irow, icol].set_ylim(
                1e-5, max([content.max() for content in tag_content.values()])*100)
            axs[irow, icol].set_yscale('log')
            axs[irow, icol].legend(loc='upper left')
    fig.savefig(f"{output_dir}/prediction_{suffix}_{split}.pdf", dpi=100)
    ...
    modelName = f"model_{split}"
    model.save(output_dir+"/"+modelName)
    print(f"Saved model to {output_dir} as {modelName}")

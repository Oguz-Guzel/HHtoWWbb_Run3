...
def muon_x(mu):
    ...
    return op.min(
        op.max(0.0, (0.9 * mu.pt * (1 + mu.jetRelIso)) - 20.0) / (45.0 - 20.0), 1.0
    )
...
def muon_pNetInterpIfMvaFailed(mu):
    ...
    return op.OR(
        op.NOT(hasAssociatedJet(mu)), mu.jet.btagPNetB < muon_btagInterpolation(mu)
    )
...
def jetIdCorrection(j):
    ...
    return op.multiSwitch(
        (op.abs(j.eta) <= 2.7, passJetIdTight),
        (
    ...
    return op.select(
        ...
        lambda mu: op.AND(
            ...
            op.OR(
                ...
                op.AND(mu.jetRelIso < 0.8, muon_pNetInterpIfMvaFailed(mu)),
            ),
        ),
    )
...
def cleanElectrons(electrons, muons):
    ...
    cleanedElectrons = op.select(
        electrons,
        lambda el: op.NOT(op.rng_any(muons, lambda mu: op.deltaR(el.p4, mu.p4) <= 0.3)),
    )
...
def ak4MediumBtagWP(era):
    ...
    WPs = {"2022": 0.245, "2022EE": 0.2605, "2023": 0.1917, "2023BPix": 0.1919}
    return WPs[era]
...
def ak4TightBtagWP(era):
    ...
    https://btv-wiki.docs.cern.ch/ScaleFactors/"""
    WPs = {"2022": 0.6734, "2022EE": 0.6915, "2023": 0.6172, "2023BPix": 0.6133}
    return WPs[era]
...
def cleaningWithRespectToLeadingLeptons(electrons, muons, DR):
    ...
    return lambda j: op.multiSwitch(
        ...
        (
            ...
            op.AND(
                op.deltaR(j.p4, electrons[0].p4) >= DR,
                op.deltaR(j.p4, electrons[1].p4) >= DR,
            ),
        ),
        # Only muons
        (
            ...
            op.AND(
                op.deltaR(j.p4, muons[0].p4) >= DR, op.deltaR(j.p4, muons[1].p4) >= DR
            ),
        ),
        ...
        (
            ...
            op.switch(
                ...
                op.switch(
                    ...
                    op.switch(
                        muons[1].pt > electrons[0].pt,
                        op.AND(
                            op.deltaR(j.p4, muons[0].p4) >= DR,
                            op.deltaR(j.p4, muons[1].p4) >= DR,
                        ),
                        op.AND(
                            op.deltaR(j.p4, muons[0].p4) >= DR,
                            op.deltaR(j.p4, electrons[0].p4) >= DR,
...
def VBFjetDef(jets):
    ...
    return op.select(
        ...
        lambda jet: op.AND(
            ...
            jet.pt >= 30.0,
            op.abs(jet.eta) <= 4.7,
            op.OR(jet.pt >= 60.0, op.abs(jet.eta) < 2.7, op.abs(jet.eta) > 3.0),
            jet.btagPNetB >= 0,
        ),
    )
...
def cleanVBFAk4_lambda(ak4jetsbybtag):
    ...
    return lambda j: op.multiSwitch(
        (
            op.rng_len(ak4jetsbybtag) > 1,
            op.AND(
                op.deltaR(j.p4, ak4jetsbybtag[0].p4) > 0.8,
                op.deltaR(j.p4, ak4jetsbybtag[1].p4) > 0.8,
            ),
        ),
        (op.rng_len(ak4jetsbybtag) == 1, op.deltaR(j.p4, ak4jetsbybtag[0].p4) > 0.8),
        op.c_bool(True),
    )
...
def VBFpair_lambda(j1, j2):
    """VBF pair selection"""
    return op.AND(
        op.invariant_mass(j1.p4, j2.p4) > 500.0,
...
def defineObjects(self, tree):
    ...
    self.VBFjetsResolved = op.select(VBFjets, cleanVBFAk4_lambda(ak4jetsbybtag))
    ...
    self.VBFjetPairsResolved = op.sort(
        op.combine(self.VBFjetsResolved, N=2, pred=VBFpair_lambda),
        lambda pair: -op.invariant_mass(pair[0].p4, pair[1].p4),
    )

    self.VBFjetPairsBoosted = op.sort(
        op.combine(self.VBFjetsBoosted, N=2, pred=VBFpair_lambda),
        lambda pair: -op.invariant_mass(pair[0].p4, pair[1].p4),
    )
...
def ml_input_features(self):
    ...
    def get_jet_callable(var, idx, jet):
        ...
        return op.switch(
            op.rng_len(jet) > idx, getattr(jet[idx].p4, var)(), op.c_float(0)
        )

    def get_jet_tree_var(var, idx, jet):
        "Similar to get_jet_callable but returns the tree variable"
        return op.switch(op.rng_len(jet) > idx, getattr(jet[idx], var), op.c_float(0.0))

    HT = op.rng_sum(self.ak4Jets, lambda jet: jet.pt)
